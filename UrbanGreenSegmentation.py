...
class UrbanGreenSegmentation_Interpolation(nn.Module):
    def __init__(self, in_channel:int=6, out_channel:int=7):
        super(UrbanGreenSegmentation_Interpolation, self).__init__()
        
        # 3개 배치 사용시 메모리 5기가
        # 2개 배치 사용시 메모리 3.8기가

        self.unet = neuralnet.UNet_Interpolation(in_channel=in_channel)
        #self.regression = neuralnet.Splitted_Regression()
...
def train_category_5_Interpolate_DataParallel(gpu_list:str):
    # --- GPU selection --- #
    gpus = str(gpu_list) # slot number (e.g., 3), no gpu use -> write just ' '
    os.environ["CUDA_DEVICE_ORDER"]="PCI_BUS_ID"
    os.environ["CUDA_VISIBLE_DEVICES"]=str(gpus)
...
def train_category_7_DataParallel(gpu_list:str):
    ...
    Datasets_ver3 = {
        ...
        'Prediction' : dataprepare.TrainDataset4(raw_data_array, OHE_target_array, raw_target_array, patch_size = patch_size, is_evaluating = True, train_ratio = train_ratio, categories=categories)
    }
    Dataloaders_ver3 = {
        'Train' : DataLoader(Datasets_ver3['Train'], batch_size=batch_size, num_workers=num_workers),
        'Validation' : DataLoader(Datasets_ver3['Validation'], batch_size=batch_size, num_workers=num_workers),
    ...
    model = UrbanGreenSegmentation(out_channel=categories)
    model = nn.DataParallel(model)
    criterion3 = nn.CrossEntropyLoss()
    #옵티마이저 바꿔보기
    optimizer3 = torch.optim.Adam(model.parameters(), lr=0.001)
    ...
    region =  str(input("Enter Region (Default : N11) :")) or "N11"
    ...
    raw_data_array_N11 ,raw_target_array_N11, OHE_target_array_N11 = dataprepare.prepare_raw_files(region, categories=7)
    N11_prediction_dataset = dataprepare.TrainDataset4(raw_data_array_N11, OHE_target_array_N11, raw_target_array_N11, patch_size = patch_size, is_evaluating = True, train_ratio = train_ratio, categories=categories)
    N11_prediction_dataloader = DataLoader(N11_prediction_dataset, batch_size=2)
    reference_data = f'/home/bcyoon/Byeongchan/Data/{region}/{region}_lidar.tif'
    result_path = legacytraining.save_result2(model.to('cpu'), dataloader=N11_prediction_dataloader, path=f'../Data/{region}/Model/Segmentation/', description=description, reference_data=reference_data, patch_size=patch_size, device = device, categories=categories)
    return f'Best Model Path : {best_model_path}\nResult Path : {result_path}'
