...
def prepare_raw_files(region:str, categories = 7):
    ...
    print('Normalizing...')
    for i in tqdm.trange(train_array.shape[0]):
        train_array[i,:,:] = (train_array[i,:,:]-np.mean(train_array[i,:,:]))/(2*np.std(train_array[i,:,:])) + 0.5
...
class TrainDataset4(Dataset):
    def __init__(self, data_array : Type[np.ndarray], target_array_OHE : Type[np.ndarray], target_array_RAW : Type[np.ndarray], patch_size : int, is_evaluating : bool = False, is_validating : bool = False, rotate : bool = False, train_ratio : float = 0.8, categories = 7):
        ...
        #self.label_OHE = np.zeros(((data_array.shape[1]//patch_size) * (data_array.shape[2]//patch_size),patch_size, patch_size), dtype=float)
        self.label_OHE = np.random.randn((data_array.shape[1]//patch_size) * (data_array.shape[2]//patch_size),patch_size, patch_size)
        ...
        for k in range(0,data_array.shape[1]//patch_size):
            for l in range(0,data_array.shape[2]//patch_size):
                ...
                #print(f'Migrating data from {k*patch_size}:{(k+1)*patch_size},{l*patch_size}:{(l+1)*patch_size} to {data_array.shape[2]//patch_size*k+l}')
        ...
        if not is_evaluating:
            if rotate:
                for i in range(2):
                    ...
                    self.label_OHE = np.concatenate((self.label_OHE, rotated_label_OHE), axis=0)
                    self.label_RAW = np.concatenate((self.label_RAW, rotated_label_RAW), axis=0)
                    print(f'Shape of label after {i} rotation : {self.label_OHE.shape}')
    ...
    def __getitem__(self, idx):
        if self.is_evaluating:
            data_reg = (self.data_reg[idx,:,:,:])
            data_seg = (self.data_seg[idx,:,:,:])
            label_OHE = (self.label_OHE[idx,:])
            label_RAW = (self.label_RAW[idx,:])
            return data_seg, data_reg, label_OHE, label_RAW
        
        if self.is_validating:
            data_reg = (self.data_reg[self.valid_index[idx],:,:,:])
            data_seg = (self.data_seg[self.valid_index[idx],:,:,:])
            label_OHE =(self.label_OHE[self.valid_index[idx],:])
            label_RAW =(self.label_RAW[self.valid_index[idx],:])
        else:
            data_reg = (self.data_reg[self.train_index[idx],:,:,:])
            data_seg = (self.data_seg[self.train_index[idx],:,:,:])
            label_OHE = (self.label_OHE[self.train_index[idx],:])
            label_RAW = (self.label_RAW[self.train_index[idx],:])

        return data_seg, data_reg, label_OHE, label_RAW

class TrainDataset5(Dataset):
    def __init__(self, data_array : Type[np.ndarray], target_array_OHE : Type[np.ndarray], target_array_RAW : Type[np.ndarray], patch_size : int, is_evaluating : bool = False, is_validating : bool = False, rotate : bool = False, train_ratio : float = 0.8, categories = 7):
        self.is_validating = is_validating
        self.is_evaluating = is_evaluating
        seed = 12345
